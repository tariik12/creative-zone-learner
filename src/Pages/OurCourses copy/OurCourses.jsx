...
const OurCourses = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/createCourse")
    .then(res => {
        ...
        <section className="grid grid-cols-3 gap-20 mt-20">
          {course?.map((d,i) => (
            <div className="rounded-xl shadow-md w-[400px] border-2" key={i}>
              <img className="w-full h-72 rounded-t-2xl" src={d?.image} alt="" />
              <div className="p-4">
                <h2 className="text-orange-600 text-lg font-bold">
                  All Courses
                </h2>
                <h2 className="my-3 text-2xl font-bold">{d?.courseName}</h2>
                <div className="flex gap-2">
                  <Rating
                    ...
                    readOnly
                  />
                  <p>{d?.student || 0} Students</p>
                </div>

                <div className="my-4 flex justify-between items-center">
                  <p className="text-xl text-gray-600 font-bold">
                    Course Fee {d?.courseFee}
                  </p>
                  <Link to={`/our-courses/${d?._id}`}>
